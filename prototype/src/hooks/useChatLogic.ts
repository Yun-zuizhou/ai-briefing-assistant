...
function buildActionSummary(intent: ParsedIntent): ActionSummary {
  switch (intent.type) {
    case 'add_interest':
      return {
        success: true,
...
export function useChatLogic() {
  const { 
    todos, setTodos,
    interests, addInterest, removeInterest, pushTime, setPushTime,
    addThoughtRecord 
  } = useAppContext();
  ...
  const processIntent = useCallback((intent: ParsedIntent, userMessage: string) => {
    addThoughtRecord(userMessage);
    
    switch (intent.type) {
      case 'add_interest': {
        const newInterests = intent.entities.interests as string[];
        newInterests.forEach((name) => addInterest(name));
        break;
      }
      case 'remove_interest': {
        const removedInterests = intent.entities.interests as string[];
        removedInterests.forEach((name) => removeInterest(name));
        break;
      }
      case 'create_todo': {
  ...
  }, [addInterest, removeInterest, setTodos, setPushTime, addThoughtRecord]);

  const generateFallbackAssistantResponse = useCallback((intent: ParsedIntent) => {
    const todoProgress = {
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (response.data) {
        ...
        intent = parseIntent(userMessage, interests);
      }
    } catch (error) {
    ...
    try {
      ...
      console.error('Chat execute error, falling back to local:', error);
    }

    if (!actionSummary) {
      processIntent(intent, userMessage);
      actionSummary = buildActionSummary(intent);
      setPendingConfirmation(null);
      assistantResponse = buildAssistantReplyFromActionSummary(actionSummary);
    }
    ...
    if (!assistantResponse) {
      assistantResponse = generateFallbackAssistantResponse(intent);
    }
    ...
    return intent;
  }, [currentSessionId, generateFallbackAssistantResponse, interests, processIntent, refreshSessionState, updateMessages]);
