...
export default function InterestConfigPage() {
  ...
  const { addInterest, removeInterest } = useAppContext();
  ...
  const handleComplete = useCallback(async () => {
    try {
      ...
      const nextInterests = response.data?.interests ?? selectedInterests;

      nextInterests.forEach((interest) => addInterest(interest));
      interestCategories.flatMap((category) => category.items).forEach((interest) => {
  ...
  }, [addInterest, navigate, removeInterest, selectedInterests]);
