...
export function useChatLogic() {
  ...
  const sendMessage = useCallback(async (
    ...
    let executeErrorMessage: string | null = null;
    ...
    try {
      ...
      executeErrorMessage = executeResponse.error ?? null;
    ...
    if (!actionSummary) {
      ...
      setPendingConfirmation(null);
      assistantResponse = executeErrorMessage
        ? `这次正式执行失败：${executeErrorMessage}`
        : buildAssistantReplyFromActionSummary(actionSummary);
    }
