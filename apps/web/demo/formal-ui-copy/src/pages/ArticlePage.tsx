...
export default function ArticlePage() {
  ...
  return (
    <PageLayout variant="secondary">
      ...
      <div style={{ height: '3px', background: 'var(--paper-warm)' }}>
        <div
          style={{
            height: '100%',
      ...
      <PageContent style={{ padding: '16px', paddingBottom: '120px' }} onScroll={handleScroll}>
        {actionError ? (
        ...
        <div style={{ padding: '16px', background: 'var(--paper-warm)', border: '2px solid var(--ink)', marginBottom: '16px', position: 'relative' }}>
          ...
          <h1 className="type-content-title" style={{ fontSize: '18px', color: 'var(--ink)', marginBottom: '12px' }}>
            {activeArticle.title}
          </h1>

          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', fontSize: '12px', color: 'var(--ink-muted)', flexWrap: 'wrap' }}>
        ...
        {activeArticle.detailState === 'partial' ? (
          <div style={{ padding: '14px', background: 'var(--paper-warm)', border: '1px dashed var(--accent)', marginBottom: '16px' }}>
            <p style={{ fontSize: '13px', color: 'var(--accent)', fontWeight: 700, margin: '0 0 6px' }}>
              当前内容仍在持续补充
            </p>
        ...
        {aiSummaryPoints.length > 0 ? (
          <div style={{ marginBottom: '16px', padding: '16px', background: 'var(--paper-warm)', border: '2px solid var(--ink)', position: 'relative' }}>
            <span style={{ position: 'absolute', top: '3px', left: '3px', right: '3px', bottom: '3px', border: '1px solid var(--ink)', pointerEvents: 'none' }} />
            <button
              ...
              style={{
                ...
                cursor: 'pointer',
                padding: 0,
                marginBottom: showAiSummary ? '10px' : 0,
              }}
            >
              <span style={{ fontSize: '14px', fontWeight: 700, color: 'var(--accent)' }}>AI 摘要</span>
        ...
        {activeArticle.summary ? (
          <div style={{ padding: '16px', background: 'var(--paper)', border: '1px solid var(--border)', marginBottom: '16px' }}>
            ...
            <p style={{ fontSize: fontSizeMap[fontSize], color: 'var(--ink-light)', lineHeight: 1.8, margin: 0 }}>{activeArticle.summary}</p>
          </div>
        ) : null}
        ...
        {activeArticle.content ? (
          <div style={{ padding: '16px', background: 'var(--paper)', border: '1px solid var(--border)', marginBottom: '16px' }}>
            ...
            <div style={{ fontSize: fontSizeMap[fontSize], color: 'var(--ink-light)', lineHeight: 1.9, whiteSpace: 'pre-wrap' }}>
              {activeArticle.content}
            </div>
          </div>
        ) : activeArticle.detailState === 'partial' ? (
          <div style={{ padding: '16px', background: 'var(--paper)', border: '1px solid var(--border)', marginBottom: '16px' }}>
            <h3 style={{ fontSize: '14px', fontWeight: 600, color: 'var(--ink)', marginBottom: '12px' }}>正文内容</h3>
            <p style={{ fontSize: '13px', color: 'var(--ink-muted)', lineHeight: 1.7, margin: 0 }}>
              ...
              {activeArticle.relatedItems.map((item) => (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', marginBottom: '6px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: 'var(--ink-muted)' }}>相关推荐</span>
                  </div>
      ...
      {showToast ? (
        <div style={{ position: 'fixed', bottom: '100px', left: '50%', transform: 'translateX(-50%)', padding: '10px 20px', background: 'var(--ink)', color: 'var(--paper)', fontSize: '13px', fontWeight: 600, fontFamily: 'var(--font-sans-cn)', border: '2px solid var(--paper)', zIndex: 1000 }}>
          {toastMessage}
        </div>
      ) : null}
    </PageLayout>
