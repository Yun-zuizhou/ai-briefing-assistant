...
export default function WeeklyReportPage() {
  ...
  return (
    <PageLayout variant="report">
      ...
      <PageContent style={{ padding: '16px' }}>
        {error ? (
          <div className="domain-card" style={{ marginBottom: '16px', padding: '14px', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '12px', color: 'var(--ink-muted)', margin: 0 }}>当前不会再展示伪周报内容，请稍后重试。</p>
          </div>
        ) : null}
        {loading ? (
          ...
          <div className="domain-card" style={{ marginTop: '16px', padding: '20px', textAlign: 'center' }}>
            <p style={{ fontSize: '14px', color: 'var(--ink)', marginBottom: '8px' }}>当前还没有正式生成的周报。</p>
            <p style={{ fontSize: '12px', color: 'var(--ink-muted)', marginBottom: '12px' }}>继续记录、收藏与行动，等真实周报生成后再查看和导出。</p>
            <button className="trend-action-btn font-sans-cn" onClick={handleRecordThought}>
              💬 去记录一条本周想法
            </button>
          </div>
        ) : (
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.4)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ width: '100%', maxWidth: '430px', background: 'var(--paper)', border: '2px solid var(--ink)', padding: '20px', position: 'relative' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', position: 'relative' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)', fontFamily: 'var(--font-sans-cn)' }}>📤 导出周报</h3>
      ...
      {reportData && showShareModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.4)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ width: '100%', maxWidth: '430px', background: 'var(--paper)', border: '2px solid var(--ink)', padding: '20px', position: 'relative' }}>
            <span style={{ position: 'absolute', top: '3px', left: '3px', right: '3px', bottom: '3px', border: '1px solid var(--ink)', pointerEvents: 'none' }} />
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', position: 'relative' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: 'var(--ink)', fontFamily: 'var(--font-sans-cn)' }}>📤 分享周报</h3>
      ...
      {toastMessage ? (
        <div style={{ position: 'fixed', bottom: '100px', left: '50%', transform: 'translateX(-50%)', padding: '12px 24px', background: 'var(--ink)', color: 'var(--paper)', fontSize: '14px', fontWeight: 600, fontFamily: 'var(--font-sans-cn)', border: '2px solid var(--paper)', zIndex: 1000, display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Check size={16} />
          {toastMessage}
        </div>
      ) : null}
