...
export async function executeInterestAction(params: {
  ...
  if (intentType === 'add_interest') {
    ...
    return {
      ...
      successMessage: '已配置关注领域',
      resultSummary: `配置成功：已新增关注 ${interests.join('、')}。后续简报会优先参考这些关注领域。`,
      nextPageLabel: '查看简报变化',
      deepLink: '/today',
      sourceContext,
    }
  }
  ...
  return {
    ...
    confirmedType: confirmedType || intentType,
    successMessage: '已配置关注领域',
    resultSummary: `配置成功：已移除关注 ${interests.join('、')}。后续简报会减少这些方向的内容。`,
    nextPageLabel: '查看简报变化',
    deepLink: '/today',
    sourceContext,
  }
