...
但 `Today / Reports` 仍停留在旧状态：

1. `dashboard.py` 仍主读 `users.interests`
2. `reports.py` 仍主读 `users.interests`
...
1. 优先读取 `user_interests`
2. 若 `user_interests` 为空，则回退读取 `users.interests`
3. 若旧字段也为空，最后再回退到虚拟兴趣 `get_virtual_interests()`

这意味着当前 Today 兴趣来源顺序已明确为：

`正式态(user_interests) -> 过渡回退(users.interests) -> 虚拟兜底(get_virtual_interests)`
...
1. 优先读取 `user_interests`
2. 若 `user_interests` 为空，则回退到 `users.interests`
